...
async def compress_news(news: NewsRequest, db: AsyncSession = Depends(get_db)):
    data = {
        "contents": [
            {
                "parts": [
                    {"text": news.body + prompt},
                ],
                "role": 'user'
            }
        ]
    ...
    try:
        ...
        status = await create_news(db=db, title=parsed_text[0], body=parsed_text[1], tags=parsed_text[2], url=news.url)
        if status != 'ok':
